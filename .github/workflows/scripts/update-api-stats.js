...
const data = require(process.env.API_SUMMARY_JSON);
...
async function appendSuiteSize() {
  const range = "Size of API Test Suite!A:B"; // Specify the range where you want to write the data
  try {
    // Obtain the client
...
async function appendExecutionTime() {
  const range = "API Suite Execution Time!A:B"; // Specify the range where you want to write the data
  try {
    // Obtain the client
    const client = await auth.getClient();
    ...
    if (duration === 0) {
      console.log("Duration is 0. Skipping the execution.");
      return;
    }
...
async function appendFailures() {
  const range = "Number of API Failures!A:B"; // Specify the range where you want to write the data
  try {
    // Obtain the client
    const client = await auth.getClient();
...
async function appendSkipped() {
  const range = "Number of Skipped API Tests!A:B"; // Specify the range where you want to write the data
  try {
    // Obtain the client
    const client = await auth.getClient();
...
async function appendBroken() {
  const range = "Number of Broken API Tests!A:B"; // Specify the range where you want to write the data
  try {
    // Obtain the client
    const client = await auth.getClient();
